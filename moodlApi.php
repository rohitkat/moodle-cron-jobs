...
global $DB;
    ...
    if ($group) {
        ...
        $groupdata = moodleAddUserGroup($id, $group);
    }

}

function moodleAddUserGroup($userid, $group)
{
    global $DB;
...
if (isset($_POST['add_type']) && $_POST['add_type'] == 'droped_reenrolled') {
    ...
    $email = 'angelinamendoza@gurnick.edu';
	...
	if($user){		
			...
			if (isset($_POST['type']) && $_POST['type'] == 'student_reenroll') {				
				...
				foreach ($drop_data as $key => $course) {
					$dropdata = userDropped($user->id, $course->courseid, $user->firstname);
					if(!$dropdata)
					$notInCourse[]=$course->coursename;
				}
				if ($dropdata) {
				...
				if($notInCourse){					
					$recc = new stdClass();
					$recc->msg = $email . "This user is not exist in courses ".implode(", ",$notInCourse).".";
					$recc->type = "gegi";
		...
		$recc = new stdClass();
		$recc->msg = $email . "This user is not exist in moodle system.";
		$recc->type = "gegi";
		$recc->timecreate = time();
		$recc->email = $email;		
		sendErrorMail($recc);
...
function enroledUser($id)
{
	$course = $DB->get_record('course', ['id' => 43]);	
	...
	if($course){
		...
		$recc = new stdClass();
		$recc->msg = $user->email . "This user have not any course in moodle system.";
		$recc->type = "gegi";
		$recc->timecreate = time();
